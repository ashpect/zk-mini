...
def groth_poc(r1cs, w):
    ...
    tau_g1 = [multiply(G1, tau**i) for i in range(deg,-1,-1)]
    tau_g2 = [multiply(G2, tau**i) for i in range(deg,-1,-1)] 
    test =  [tau**i for i in range(deg,-1,-1)] 
    # print("test", test)
    ...
    # print("tau_g1", len(tau_g1))
    # print("tau_g2", len(tau_g2))
    # print("ht_srs", ht_srs)
    ...
    Lw_qap_coeffs = [0] + [int(x) for x in Lw_qap.coeffs]
    Rw_qap_coeffs = [0] + [int(x) for x in Rw_qap.coeffs]
    Ow_qap_coeffs = [0] + [int(x) for x in Ow_qap.coeffs]
...
def prover_grothv0(r1cs, w):
    ...
    print("tau_g1", len(tau_g1))
    print("tau_g2", len(tau_g2))
    print("ht_srs", len(ht_srs))
    print("phi_srs", len(phi_srs))

    A = add(alpha_g1, inner_product(tau_g1, Lw_qap_coeffs))
    B = add(beta_g2, inner_product(tau_g2, Rw_qap_coeffs)) 
    C_part1 = inner_product(ht_srs, h_qap_coeffs) 
    C_part2 = inner_product(phi_srs, w.matrix)
